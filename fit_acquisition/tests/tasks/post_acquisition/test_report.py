...
def test_init_report_task(task_instance):
    task, _ = task_instance

    assert task.label == translations["REPORTFILE"]
    assert task.state == State.INITIALIZATED
    assert task.status == Status.SUCCESS
    assert task.progress_bar.value() == 0


def test_report_task(task_instance, qtbot, test_folder):
    ...
    assert ui.statusbar.currentMessage() == translations[
        "GENERATE_PDF_REPORT_COMPLETED"
    ].format(Status.SUCCESS.name)
    assert task.progress_bar.value() == 100

    assert os.path.exists(os.path.join(test_folder, "acquisition_report.pdf"))
