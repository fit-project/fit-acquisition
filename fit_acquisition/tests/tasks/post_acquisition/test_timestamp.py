...
def test_folder():
    folder = resolve_path("acquisition/tasks/timestamp")
    os.makedirs(folder, exist_ok=True)
    return folder


@pytest.fixture(scope="module")
def test_pdf_filename(test_folder):
    file = open(os.path.join(test_folder, "acquisition_report.pdf"), "w")
...
@pytest.fixture
def task_instance(main_window, test_folder, test_pdf_filename):
...
def test_timestamp_task(task_instance, qtbot, test_folder):
    ...
    assert os.path.exists(os.path.join(test_folder, "timestamp.tsr"))
    assert os.path.exists(os.path.join(test_folder, "tsa.crt"))
