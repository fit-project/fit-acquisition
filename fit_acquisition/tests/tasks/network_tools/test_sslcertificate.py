...
def test_sslcertificate_task(task_instance, qtbot):
    ...
    assert (
        ...
        == translations["SSLCERTIFICATE_STARTED"]
    )
    assert task.progress_bar.value() == 0
    ...
    assert task.status == Status.SUCCESS
    assert ui.statusbar.currentMessage() == translations["SSLCERTIFICATE_GET_FROM_URL"].format(task.status.name, task.options["url"])
